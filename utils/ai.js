  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: 'user', content: `Date: ${date}\nInterests: ${kind}\nWeather: ${weather}\nLocation: ${city}, ${country}\nRequest: Based on the above information, please provide 4 travel destinations with name, address, description, latitude, and longitude included. Please provide the information in Korean.` },
      ]
    });
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: 'user', content: `Date: ${date}\nInterests: ${kind}\nWeather: ${weather}\nLocation: ${city}, ${country}\nRequest:차례 번호 없이, 레스토랑의 이름, 위치, 주소, 설명, 위도 및 경도를 포함하여 좋은 레스토랑 5곳의 정보를 한국어로 알려주세요.` },
      ]
    });
